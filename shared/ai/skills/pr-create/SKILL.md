...
2. 現在の `head branch` と `base branch` を確認する
3. PR に含める変更の要点、影響範囲、確認項目を整理する
4. 変更内容に沿って PR タイトルと本文を作る
5. ユーザーに実行コマンドの確認を求める
6. `gh pr create --base <base> --head <head>` で PR を作成する
...
## Testing
Changesに対して実行した確認方法を番号付きで列挙

## Notes
注意点やレビューで見てほしい点を番号付きで列挙
```

## Constraints

- 変更内容を確認してから、タイトルや本文を決める
- PR に無関係な差分が混ざっていないか確認する
- 不明な点は推測で埋めず、本文に前提や制約として書く
- テスト未実施なら、その旨を `Testing` に明記する
- `base branch` と `head branch` を確認してから PR を作成する
